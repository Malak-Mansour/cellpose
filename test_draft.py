...
# # image_dir = Path("../flattened_dataset/all_images") #test model of all images(without chaining)
# image_dir = Path("../filtered_DEP_data/MDA_static_L4/Images") #cross-domain MDA inference
# # image_dir = Path("../filtered_DEP_data/6. SKBR3_ON_OFF_10V_R1/Images") #test model of all images(with chaining)

# # image_files = sorted([f for f in image_dir.glob("t*.tif")])
# # image_files = sorted([f for f in image_dir.glob("t*.tif")])[:2]
# image_files = sorted([f for f in image_dir.glob("t*.tif")])[-6:]
# # image_files = sorted([f for f in image_dir.glob("img_*.tif")])[-6:]
...
# custom_model_path = "/l/users/malak.mansour/DEP/cellpose/models/fine_model_all_without_chaining"
# # custom_model_path = "/l/users/malak.mansour/DEP/cellpose/models/fine_model_all_chaining"
...
# viz_dir = Path("overlay_visualizations/MDA")
# # viz_dir = Path("overlay_visualizations/chaining")
...
python test.py \
  ...
  --img_dir "../filtered_DEP_data/6. SKBR3_ON_OFF_10V_R1/Images/L6" \
  --img_pattern "t*.tif" \
  --img_slice ":3" \
...
python test.py \
  --model_name "/l/users/malak.mansour/DEP/cellpose/models/fine_model_all_chaining" \
  --img_dir "../filtered_DEP_data/3. SKBR3_Static_R1/Images/L3" \
  --img_pattern "t*.tif" \
  --img_slice ":3" \
  --overlay_folder "overlay_visualizations/Chaining"
